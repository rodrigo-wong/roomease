...
export default function Dashboard({
    ...
    const {
        ...
        setData: setAdminBookingData,
        post: postAdminBooking,
        processing: adminBookingProcessing,
        errors: adminBookingErrors,
        reset: resetAdminBookingForm,
    } = useForm({
        room_id: "",
    ...
    const getItemName = (bookable) => {
        if (bookable.bookable) {
            ...
            if (bookable.bookable_type.includes("Contractor")) {
                ...
                const role = contractorRoles?.find(
                    (r) => r.id === bookable.bookable.role_id
                );
        ...
        if (bookable.bookable_type.includes("Product")) {
            ...
            const product = products.find(
                (product) => product.id === bookable.bookable_id
            );
    ...
    const getContractorsByRole = (roleId) => {
        return contractors.filter(
            (contractor) => contractor.role_id === roleId
        );
    };
    ...
    const getBookingTimeDisplay = (order) => {
        ...
        return `${formatDate(startTime)}, ${formatTime(
            startTime
        )} - ${formatTime(endTime)}`;
    };
    ...
    const handleAdminBookingSubmit = (e) => {
        ...
        if (adminBookingData.start_time && adminBookingData.end_time) {
            const startDateTime = new Date(
                `${adminBookingData.date}T${adminBookingData.start_time}`
            );
            const endDateTime = new Date(
                `${adminBookingData.date}T${adminBookingData.end_time}`
            );
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="mb-4">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setData("contractor_id", e.target.value)
                                    }
                                                        ...
                                                        (bookable) => (
                                                            ...
                                                            >
                                                                <td className="px-4 py-2">
                                                                    {bookable.bookable_type.includes(
                                                                        "ContractorRole"
                                                                    )
                                                                        ? "Contractor"
                                ...
                                <div>
                                    ...
                                    >
                                        ...
                                        {rooms &&
                                            rooms.map((room) => (
                                                <option
                                                    key={room.bookable_id}
                                                    value={room.bookable_id}
                                                >
                                                    {room.name}
                                                </option>
                                            ))}
